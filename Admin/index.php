...
<aside id="sidebar" class="column">
		...
		<h3>Reports:</h3>
		<ul class="toggle">
		    <li class="icn_settings"><a href="a_report.php" target="_blank">Order Report</a></li>
	...
	<section id="main" class="column">
		...
		<h4 class="alert_info">Welcome To <strong>"HFND.COM"</strong> Admin Panel As: <?php echo "  ". "<font color='#f90'><big><b>".$login_session."</b></big></font>" ;?>  </h4> 
		...
		<article class="module width_full">
			...
			<div class="module_content">
				...
				</article>	<article class="stats_overview">
					<div class="overview_today">
						<p class="overview_day">Yesterday</p>
						<p class="overview_count">				<?php
$qurys=mysqli_query($mysqli,"select sum(Total_Amount) AS TotalItemsOrdered FROM payment where date(payment.date)=CURDATE()-1");
$rw=mysqli_fetch_array($qurys);
?><?php echo number_format($rw['TotalItemsOrdered'],2);?></span></p>
						<p class="overview_day">Today's Total Sales</p>
						<p class="overview_count"><?php
$quryss=mysqli_query($mysqli,"select sum(Total_Amount) AS TotalItemsOrdered FROM payment where date(payment.date)=CURDATE()");
$rws=mysqli_fetch_array($quryss);
?><?php echo number_format($rws['TotalItemsOrdered'],2);?></span></p>
						
					</div>
					<div class="overview_previous">
						<p class="overview_day">Total Sales</p>
						<p class="overview_count">						<?php

$query=mysqli_query($mysqli,"select sum(Total_Amount) AS TotalItemsOrdered FROM payment");
$row=mysqli_fetch_array($query);
?><?php echo number_format($row['TotalItemsOrdered'],2);?></span></p>
						<p class="overview_day">Last 7 Days Sales</p>
						<p class="overview_count">					<?php
$qury=mysqli_query($mysqli,"select sum(Total_Amount) AS TotalItemsOrdered FROM payment where date(payment.date)>=(DATE(NOW()) - INTERVAL 7 DAY)");
$row=mysqli_fetch_array($qury);
?><?php echo number_format($row['TotalItemsOrdered'],2);?></span>
<font size="3">$</small></span></p>
			</p>
...
<?php while($row = mysqli_fetch_array($result))
  ...
  <?php }mysqli_close($mysqli);?>
	     ...
	     <div class="shout_box">
            <div class="header"> live Discussion of HFND.COM <div class="close_btn">&nbsp;</div></div>
