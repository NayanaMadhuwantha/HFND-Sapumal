...
$query=mysqli_query($mysqli,"select month(payment.date) as mnth,year(payment.date) as yearr,sum(payment.qty*payment.Price) as tt  from payment join payment on payment.id=payment.Product_Id  where date(payment.date) between '$fdate' and '$tdate' group by mnth,yearr");
